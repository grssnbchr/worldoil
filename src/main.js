...
/* Parameters */
var LONCENTER = 10;                     // longitude of the map center
var LATCENTER = 30;                     // latitude of the map center
var MAPSCALE = 210;                     // display scale of the map center
var MAPTRANSLATEX = 500;                // horizontal translation of map
var MAPTRANSLATEY = 50;                 // vertical translation of map

var BARCHARTTRANSLATEX = -50;           // horizontal translation of barchart
var BARCHARTTRANSLATEY = 200;           // vertical translation of barchart
var MAXBARCHARTWIDTH = 200;             // maximum width of barchart (for both production and consumption)

var TSTRANSLATEX = 60;                  // horizontal translation of timeseries chart
var TSTRANSLATEY = 560;                 // vertical translation of timeseries chart

var MAXCIRCLERADIUS = 60;               // maximum radius of halfcircles 


var TRANSDURATION = 1000;               // duration of transitions in ms
var TOOLTIPTRANSDURATION = 250;         // duration of fading for tooltips in ms
var BARCHARTSHOWONLY = 15;              // number of barcharts to be shown (attention: affects geometry of display, so other parameters need to be adjusted)
...
// default year, can be changed
var year = '2012';
// default barchart order, can be changed
 ...
 * Loads the data which triggers the rest of the chain
 */
var init = function() {
    // create one big svg panel
...
/*
 * UI FUNCTIONS 
 * These functions are only executed once at the beginning of the script
 */

/**
 * Creates the control panel on top
 * In particular, injects div elements into the container div that
 * contain the slider and the sort order selection
 */
...
/**
 * Creates the impress (displays it)
 */
 ...
 * After the data is loaded, prepareData is called
 * 
 */
 ...
 * geometries to the data, once all data is loaded
 * Once the data is prepared, the displayData function is called
 */
var prepareData = function(error, world, consumption, production, names){
    ...
    for(var i = 1965; i <= 2012; i++){
        ...
        consumption.forEach(function(d,j){
            ...
            var country = {'name': name, 'id': parseInt(d.id), 'consumption': parseInt(d[i + '']), 'production': parseInt(production[j][i + ''])};
            if(typeOf(parseInt(country.production)) == 'number' || typeOf(parseInt(country.consumption)) == 'number')
                countryarray.push(country);
        });
        data[i + ''] = countryarray;
    }
    // dataByCountry is another form of the data object, where all years for every country are stored, ie.
    // the country name is the index and the value is an array of all the years
    // this is used for the timeseries only
    // one might ask why the country name and not the country id is used for indexing the countries, this is because
    // the country name is needed as well and can't be otherwise stored using this primitive data structure 
    dataByCountry = {};
...
/**
 * Draws the initial map without any thematic data, i.e. displays the topojson country data
 * Specifies projection etc.
 * Already adds SVG groups for half-circles and clippath definitions (the clippaths are used for the production of
 *     half-circles)
 */
 ...
 * These functions are re-executed after each user interaction, ie. after each setting of a new year or
 * a new sorting order
 */
 ...
 * 
 * This function seems to be pretty long, but this is only because
 * the logic for the data about individual countries and for the data
 * about the whole world is different
 */
...
var selectCountry = function(id, showTooltip){
    ...
    // get the data from the DOM, since countries cannot be accessed through IDs in either the data or the dataByCountry objects
    // this is a bit complicated, but the easiest solution
    ...
    // construct data object for which a tooltip will be added
    ...
    if(id !== 900 && data !== undefined && dataByCountry[data.name] !== undefined){
...
function getCentroid(id){
    ...
    // the following is for cases where an object in the data has no counterpart in the geometry (for example Hong Kong and Singapore, for some reason)
...
/** 
 * Helper function
 * Creates the rest of the display (UI and Impress) and destroys the loading screen
 */
